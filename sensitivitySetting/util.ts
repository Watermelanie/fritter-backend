...
const constructSensitivitySettingResponse = (sensitivitySetting: HydratedDocument<SensitivitySetting>): SensitivitySettingResponse => {
  ...
  const {username} = sensitivitySettingCopy.user;
  delete sensitivitySettingCopy.user;
  return {
    ...sensitivitySettingCopy,
    _id: sensitivitySettingCopy._id.toString(),
    user: username,
    sensitiveContent: sensitivitySetting.sensitiveContent,
    hiddenFreet: sensitivitySetting.hiddenFreet
  };
};

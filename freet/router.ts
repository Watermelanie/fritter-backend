...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (wordsDetected.size === 0) {
      ...
      const contentFilter = await ContentFilterCollection.addOne(freet._id, false);
...
/**
 * Create a new freet after warning about contents.
 *
 * @name POST /api/freets/warning
 *
 * @param {string} content - The content of the freet
 * @return {FreetResponse} - The created freet
 * @throws {403} - If the user is not logged in
...
router.post(
  ...
  async (req: Request, res: Response) => {
    const userId = (req.session.userId as string) ?? ''; // Will not be an empty string since its validated in isUserLoggedIn
    const freet = await FreetCollection.addOne(userId, req.body.content);
    const contentFilter = await ContentFilterCollection.addOne(freet._id, true);
...
/**
 * Modify a freet after warning of inappropriate content
 *
 * @name PUT /api/freets/:id/warning
 *
 * @param {string} content - the new content for the freet
 * @return {FreetResponse} - the updated freet
 * @throws {403} - if the user is not logged in or not the author of
...
router.put(
  ...
  async (req: Request, res: Response) => {
    const freet = await FreetCollection.updateOne(req.params.freetId, req.body.content);
    await ContentFilterCollection.updateStatus(freet._id, true);
    res.status(200).json({
      message: 'Your freet was updated successfully.',
      freet: util.constructFreetResponse(freet)
    });
